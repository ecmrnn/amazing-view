        ...
        <div class="flex items-center justify-between gap-3 sm:items-start">
            ...
            <div class="flex items-start gap-1">
                ...
                @if (!empty($reservation->invoice))
                    @if ($reservation->status == App\Models\Reservation::STATUS_CHECKED_OUT || $reservation->status == App\Models\Reservation::STATUS_COMPLETED)
                        @if ($reservation->invoice->issue_date == null)
                            <a href="{{ route('app.billings.create', ['rid' => $reservation->rid]) }}" wire:navigate.hover>
                                <x-primary-button class="hidden text-xs md:block">Create Invoice</x-primary-button>
                            </a>
                        @else
                            <a href="{{ route('app.billings.show', ['billing' => $reservation->invoice->iid]) }}" wire:navigate.hover>

    ...
    <div class="max-w-screen-lg mx-auto space-y-5">
        <div class="flex items-center justify-between p-5 bg-white border rounded-lg border-slate-200">
            ...
            @if ($page->title != 'Global')
                <div class="flex items-center gap-5">
                    <livewire:app.content.change-status :page="$page" />
                    
                    <x-actions>
                        <div class="space-y-1">
                            @if ($page->title != 'Home')
                                <x-action-button x-on:click="$dispatch('open-modal', 'change-status-modal'); dropdown = false;">
        ...
        <x-note>
            <p>Any update made on this page will be automatically applied to the website. 
                @if ($page->title != 'Global')
                    You may view what your changes may look like using the preview in the action button.
                @endif

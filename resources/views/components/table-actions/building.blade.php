<div wire:key="{{ $row->id }}" class="flex justify-end gap-1">
    ...
    @if (empty($row->rooms->count()))
        <x-tooltip text="Delete" dir="top">
            <x-icon-button x-ref="content" x-on:click="$dispatch('open-modal', 'delete-building-table-{{ $row->id }}-modal')">
    ...
    {{-- Modals --}}
    <x-modal.full name='delete-building-table-{{ $row->id }}-modal' maxWidth='sm'>
        <form class="p-5 space-y-5" x-on:building-deleted.window="show = false" wire:submit="delete({{ $row->id }})">
            <hgroup>

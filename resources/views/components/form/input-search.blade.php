...
<div x-data="{ focus: false }"
    ...
    class="z-0 flex items-center pl-3 text-xs text-gray-900 transition-colors duration-200 ease-in-out bg-transparent bg-white border rounded-lg appearance-none border-1 focus:outline-none focus:ring-0 focus:border-blue-600 peer invalid:border-red-500 invalid:bg-red-50 focus:invalid:border-red-500">
    ...
    <input {{ $disabled ? 'disabled' : '' }} {{ $attributes->merge(['class' => 'border-0 text-xs py-2 rounded-lg focus:ring-0 font-semibold']) }} type="{{ $type }}"
